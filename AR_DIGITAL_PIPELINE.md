...
- **WhatsApp vínculo de selo:** telefones brasileiros recebidos como `119...`, `+5511...`, `5511...` ou via `@lid` devem ser normalizados para `55...` antes da busca em `whatsapp_identities`. Quando um telefone sem vínculo manda `oi`, o bot pede o número de registro do selo, por exemplo `A7K9`; ao receber o registro válido, salva o telefone no selo e não pergunta novamente. A qualquer momento, `trocar selo` apaga o vínculo atual, expira sessões abertas e pede um novo registro.
- **WhatsApp reinício por código:** se o usuário mandar um registro válido de selo como `A7K9` ou `#A7K9`, o webhook deve expirar sessões antigas desse telefone, reiniciar o intake e mandar a primeira pergunta do fluxo mesmo que o telefone já esteja vinculado ou exista uma sessão ativa.
- **WhatsApp resposta via Evolution:** Cloudflare Pages não deve chamar a Evolution por IP direto/porta `8080`. Produção usa `EVOLUTION_BASE_URL=https://evolution.193-203-182-39.sslip.io`, roteado pelo Caddy da VPS para `localhost:8080` em HTTPS/443. O webhook retorna `reply_failed` com `error_code` seguro se a resposta ao WhatsApp não for enviada, evitando falso sucesso.
